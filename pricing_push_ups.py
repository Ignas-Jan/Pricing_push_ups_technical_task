...
print(f"Times that push-ups has been used by seller: {int(times_of_push_ups_used)}")
print("-" * 150)
...
grouped_by_category2_ratio["revenue_per_listing"] = (
    grouped_by_category2_ratio["revenue_from_push_ups"] / grouped_by_category2_ratio["number_of_listings"]
).round(4)
...
ax1.set_xlabel("Push-up Adoption Rate (%)", fontsize=12, fontweight="bold")
...
ax2.set_xlabel("Push-Up Adoption Rate (%)", fontsize=12, fontweight="bold")
...
plt.title("Correlation between Push-up Cost Burden ratio and Push-Up Adoption Rate")
plt.xlabel("Push-Up Cost Burden (% of Price)")
plt.ylabel("Push-up Adoption Rate (%)")
...
ax1.set_xlabel("Push-Up Cost Burden ratio (%)", fontsize=12, fontweight="bold")
ax1.set_title("Top 5 Categories with highest Push-Up Cost Burden ratio", fontsize=14, fontweight="bold", pad=20)
ax1.invert_yaxis()
...
for i, (bar, rate, percentage, price) in enumerate(zip(bars_top, top5_1["adoption_rate_%"], 
    ...
    ax1.text(bar.get_width() * 0.5, bar.get_y() + bar.get_height()/2, 
            f"Adoption rate: {rate:.2f}%\nCost Burden ratio: {percentage:.2f}%\nAverage price: {price:.2f}€",
            va="center", ha="center", fontsize=10, fontweight="bold", color="white")
...
ax2.set_xlabel("Push-Up Cost Burden ratio (%)", fontsize=12, fontweight="bold")
ax2.set_title("Bottom 5 Categories with lowest Push-Up Cost Burden ratio", fontsize=14, fontweight="bold", pad=20)
ax2.invert_yaxis()
...
#revenue_per_listing = total_revenue_from_push_ups / total_listings
#print(revenue_per_listing)

#grouped_by_revenue_per_listing = pricing_data_df.groupby("category_2").agg({
#    "number_of_listings" : "sum",
#    "revenue_from_push_ups" : "sum"
#})


#grouped_by_revenue_per_listing = grouped_by_revenue_per_listing.sort_values("revenue_per_listing", ascending=False)
#print(grouped_by_revenue_per_listing)